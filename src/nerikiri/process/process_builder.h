...
namespace nerikiri {
    ...
    public:
        ...
        static void preloadTopics(ProcessStore& store, const std::shared_ptr<BrokerProxyAPI>& broker, const Value& config, const std::string& path);
        static void preloadCallbacksOnStarted(ProcessStore& store, const Value& config, const std::string& path);


        static Value publishTopic(ProcessStore& store, const std::shared_ptr<BrokerProxyAPI>& broker, const Value& opInfo, const Value& topicInfo);
        static Value subscribeTopic(ProcessStore& store, const std::shared_ptr<BrokerProxyAPI>& broker, const Value& opInfo, const std::string& argName, const Value& topicInfo);
    };
}